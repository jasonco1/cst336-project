$(document).ready(function() {
    ...
    // Print Starting Album List Report
    // For use in Brian's debugging
    printTempArray();
    
    function printTempArray() {
        var parsed = "";
        for (i = 0; i< albumsArray.length; i++) {
            var tempObj=  albumsArray[i];
            for (var property in tempObj) {
                parsed += property + ": " + tempObj[property] + "\n";  
            }
        }
        $("#report-display").val(parsed); // Display printed report on text field.
    }
    ...
    {  
        ...
        deletion.value = " ";               
        printTempArray(); // View the new array without the deleted item.
    } 
